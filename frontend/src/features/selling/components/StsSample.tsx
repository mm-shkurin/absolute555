...
export function StsSample({ good }: { good: boolean }) {
  return (
    <figure className={styles.sample} data-sample={good ? 'good' : 'bad'}>
      <svg viewBox="0 0 160 120" role="img" aria-label={good ? 'СТС ровно и целиком' : 'СТС с бликом и обрезанным краем'}>
        ...
        <circle cx="146" cy="16" r="10" fill={good ? 'var(--measure-ok, #1f9d55)' : 'var(--measure-bad, #d64545)'} />
        <path
