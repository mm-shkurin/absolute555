...
function toQueueItem(item: WireQueueItem): QueueItemWire {
  return {
    ...
    // Сводка карты едет в карточке ленты: без неё модератор видел «без карты» у полной.
    measured_panels: item.thickness?.measured_panels ?? 0,
    total_panels: item.thickness?.total_panels ?? 13,
    complaints_count: item.open_complaints,
    complaint_reason: null,
    is_import: item.listing_kind === 'import',
    vin_masked: null,
