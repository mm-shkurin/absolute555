...
class SSEManager:
    ...
    def add_connection(self, car_id:str, queue:asyncio.Queue):
        ...
        logger.info("Added connection for car_id: {}", car_id)
    ...
    def remove_connection(self, car_id: str, queue: asyncio.Queue):
        if car_id in self.active_connections:
            try:
                ...
                logger.info("Removed SSE connection for car_id={}", car_id)
            except ValueError:
                pass

    async def send_message(self, car_id: str, message: Dict[str, Any]):
        logger.info("Attempting to send SSE message to car_id={}", car_id)

        try:
            channel = f"sse_messages:{car_id}"
            message_json = json.dumps(message)
            self.redis_client.publish(channel, message_json)
            logger.info("Published SSE message to Redis channel {}", channel)
        except (redis.RedisError, TypeError) as e:
            logger.error("Error publishing to Redis for car_id={}: {}", car_id, e)
        ...
        if car_id in self.active_connections:
            ...
            for queue in connections:
                try:
                    ...
                    logger.info("Sent message locally to car_id={}", car_id)
                except RuntimeError as e:
                    logger.error("Error sending local message to car_id={}: {}", car_id, e)
