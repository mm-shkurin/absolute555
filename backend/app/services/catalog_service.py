...
# Above this trigram similarity a fuzzy hit is taken as the answer; below it the
# spelling goes to a moderator instead. Set high on purpose: a wrong automatic match
# writes a confident `Toyota Camry` over a listing whose СТС said `Toyota Carina`, and
# nobody ever looks at it again. A missed match costs one queue entry.
FUZZY_ACCEPT = 0.72
...
class CatalogService:
    ...
    async def match_brand(self, raw: str | None) -> Match:
        ...
        fuzzy = await self.db.execute(
            ...
            .where(func.similarity(Brand.slug, key.lower()) >= FUZZY_ACCEPT)
    ...
    async def match_model(self, brand_id: UUID, raw: str | None) -> Match:
        ...
        fuzzy = await self.db.execute(
            ...
            .where(
                ...
                func.similarity(CarModel.slug, key.lower()) >= FUZZY_ACCEPT,
            )
            .order_by(func.similarity(CarModel.slug, key.lower()).desc())
            .limit(1)
        )
        row = fuzzy.first()
        return Match(row[0], "fuzzy") if row else Match(None, "none")
