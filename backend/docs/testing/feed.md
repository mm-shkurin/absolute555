...
### TC-FEED-001 — `GET /sale_car/list` без токена возвращает `items`, `total`, `page`, `size`

| Поле | Значение |
|---|---|
| ID | TC-FEED-001 |
| Название | `GET /sale_car/list` без токена возвращает `items`, `total`, `page`, `size` |
| История | 07 — Лента: фильтры, сортировка, пагинация, счётчик |
...
### TC-FEED-002 — В карточке ленты нет `description` и `phone_number`
...
| Тестовые данные | `<PUB>` — `sale_car_id` объявления, опубликованного по TC-LST-010 и TC-MOD-002 |
...
### TC-FEED-003 — Неопубликованное объявление не видно ни в общей, ни в отфильтрованной ленте
...
### TC-FEED-004 — `model_id` семейства находит исполнения («GS» → «GS 430»), но не соседние («GS 300»)
...
### TC-FEED-005 — Год, цена и коробка сужают ленту одновременно; концы диапазона включаются
...
### TC-FEED-006 — `sort=price_asc` / `price_desc` упорядочивают по цене; по умолчанию первым идёт последнее опубликованное
...
| Тестовые данные | `<NEWEST>` — `sale_car_id` объявления, одобренного последним (TC-MOD-002) |
...
### TC-FEED-007 — Объявление не приходит на двух страницах; страница за концом пуста, `total` сохраняется
...
### TC-FEED-008 — `size` > 60, `size` = 0 и `page` = 0 отклоняются `422 FEED_QUERY_INVALID`; `size` = 60 принимается
...
### TC-FEED-009 — `model_id` без `brand_id` и диапазон «от» > «до» отклоняются `422 FEED_QUERY_INVALID`
...
### TC-FEED-010 — Неизвестный параметр даёт `422 UNKNOWN_FILTER`; SQL в значении фильтра — просто данные
