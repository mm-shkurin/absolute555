...
### TC-REV-001 — Отзыв по сделке: 201, рейтинг продавца пересчитан
...
### TC-REV-002 — Повторный отзыв — 409 `REVIEW_ALREADY_WRITTEN` с `review_id`
...
### TC-REV-003 — Предложение не принято — 409 `DEAL_NOT_CLOSED`
...
### TC-REV-004 — Не автор предложения — 404 `OFFER_NOT_REVIEWABLE`; не UUID — 422 `MALFORMED_IDENTIFIER`
...
| Шаги | 1. `POST /offer/<OFFER_ID>/review`, `Authorization: Bearer <TOKEN_IMPORTER_I>`<br>2. `POST /offer/<OFFER_ID>/review`, `Authorization: Bearer <TOKEN_SELLER_A>`<br>3. `POST /offer/<RANDOM_UUID>/review`, `Authorization: Bearer <TOKEN_BUYER_B>`<br>4. `POST /offer/not-a-uuid/review`, `Authorization: Bearer <TOKEN_BUYER_B>` |
| Ожидаемый результат | 1–3. `404`, `code: OFFER_NOT_REVIEWABLE`<br>4. `422`, `code: MALFORMED_IDENTIFIER`, `details: {"field": "offer_id"}` |
| Статус | Not run |
| Фактический результат | |

### TC-REV-005 — Гость — 403 `GUEST_FORBIDDEN`; без авторизации — 401
...
| Предусловия | Выполнен TC-REV-001 (`<REVIEW_ID>`) |
| Тестовые данные | `{"rating": 5}` |
| Шаги | 1. `POST /offer/<OFFER_ID>/review`, `Authorization: Bearer <TOKEN_GUEST_G>`<br>2. `PATCH /review/<REVIEW_ID>`, `Authorization: Bearer <TOKEN_GUEST_G>`<br>3. `POST /offer/<OFFER_ID>/review` без `Authorization`<br>4. `PATCH /review/<REVIEW_ID>` без `Authorization` |
| Ожидаемый результат | 1–2. `403`, `code: GUEST_FORBIDDEN`<br>3–4. `401`, `code: UNAUTHENTICATED` |
| Статус | Not run |
| Фактический результат | |

### TC-REV-006 — `rating` 0, 6, отсутствует; `text` > 2000 — 422
...
| Шаги | 1. `POST /offer/<OFFER_ID>/review`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело `{"rating": 0}`<br>2. То же, `{"rating": 6}`<br>3. То же, `{"text": "без оценки"}`<br>4. То же, `{"rating": 5, "text": "<2001 символ>"}`<br>5. То же, `{"rating": 5}` |
| Ожидаемый результат | 1–4. `422`, `code: VALIDATION_ERROR`; `reviews_count` продавца = 0<br>5. `201`, `text: null` |
| Статус | Not run |
| Фактический результат | |

### TC-REV-007 — Автор правит в течение суток — 200; не автор — 404 `REVIEW_NOT_FOUND`
...
| Тестовые данные | `{"rating": 5}` |
| Шаги | 1. `PATCH /review/<REVIEW_ID>`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело из тестовых данных<br>2. `GET /seller/<ID_SELLER_A>`<br>3. `PATCH /review/<REVIEW_ID>`, `Authorization: Bearer <TOKEN_IMPORTER_I>`, тело `{"rating": 1}`<br>4. `PATCH /review/not-a-uuid`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело `{"rating": 1}` |
...
### TC-REV-008 — Спрашивавший оценивает по диалогу — 201; повтор — 409; продавец — 404 `DIALOG_NOT_REVIEWABLE`
...
| Шаги | 1. `GET /chat/dialogs`, `Authorization: Bearer <TOKEN_BUYER_B>`<br>2. `POST /chat/dialogs/<DIALOG_ID>/review`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело из тестовых данных<br>3. `POST /chat/dialogs/<DIALOG_ID>/review`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело из тестовых данных<br>4. `POST /chat/dialogs/<DIALOG_ID>/review`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"rating": 5}`<br>5. `GET /chat/dialogs`, `Authorization: Bearer <TOKEN_BUYER_B>` |
...
### TC-REV-009 — Профиль, отзывы и объявления открыты без входа; неизвестный — 404 `SELLER_NOT_FOUND`
