...
### TC-CHAT-001 — Диалог появляется у обеих сторон, первая строка — системная о цене
...
| Тестовые данные | `<DIALOG_ID>` — `dialog_id` из ответа шага 1; `<SALE_CAR_ID>` — `sale_car_id` объявления из предусловия |
...
### TC-CHAT-002 — Отправка сообщения: 201, рост `unread` у второй стороны
...
### TC-CHAT-003 — `POST /read` помечает названные сообщения, свои и повторные — не считаются
...
| Шаги | 1. `POST /chat/dialogs/<DIALOG_ID>/read`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"message_ids": ["<M1>", "<M2>"]}`<br>2. `POST /chat/dialogs/<DIALOG_ID>/read`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"message_ids": ["<M1>", "<M2>"]}`<br>3. `POST /chat/dialogs/<DIALOG_ID>/read`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"message_ids": ["<OWN_ID>"]}`<br>4. `POST /chat/dialogs/<DIALOG_ID>/read`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"message_ids": []}` |
...
### TC-CHAT-004 — История постранична; `size` вне 1..100 и `page` < 1 — 422
...
| Тестовые данные | `<DIALOG_ID>` — `dialog_id` ДИАЛОГ_AB из шага 1 TC-CHAT-001 |
...
### TC-CHAT-005 — Не участник получает 404 `DIALOG_NOT_FOUND` на чтение, запись и отметку
...
| Шаги | 1. `GET /chat/dialogs/<DIALOG_ID>/messages`, `Authorization: Bearer <TOKEN_IMPORTER_I>`<br>2. `POST /chat/dialogs/<DIALOG_ID>/messages`, `Authorization: Bearer <TOKEN_IMPORTER_I>`, тело `{"text": "Здравствуйте"}`<br>3. `POST /chat/dialogs/<DIALOG_ID>/read`, `Authorization: Bearer <TOKEN_IMPORTER_I>`, тело `{"message_ids": ["<RANDOM_UUID>"]}`<br>4. `GET /chat/dialogs/<RANDOM_UUID>/messages`, `Authorization: Bearer <TOKEN_BUYER_B>`<br>5. `GET /chat/dialogs/not-a-uuid/messages`, `Authorization: Bearer <TOKEN_BUYER_B>` |
...
### TC-CHAT-006 — Запросы без `Authorization` — 401
...
| Тестовые данные | `<DIALOG_ID>` — `dialog_id` ДИАЛОГ_AB из шага 1 TC-CHAT-001; тело шага 4 `{"text": "hi"}` |
...
### TC-CHAT-007 — Пустой текст и лишнее поле `kind` — 422
...
### TC-CHAT-008 — `POST /chat/dialogs/direct/{user_id}` открывает или находит диалог; с собой и с никем — 404
...
| Тестовые данные | `<RANDOM_UUID>` — любой новый UUID |
| Шаги | 1. `POST /chat/dialogs/direct/<ID_IMPORTER_I>`, `Authorization: Bearer <TOKEN_BUYER_B>`, без тела<br>2. `POST /chat/dialogs/direct/<ID_IMPORTER_I>`, `Authorization: Bearer <TOKEN_BUYER_B>`, без тела<br>3. `GET /chat/dialogs/<DIALOG_ID шага 1>/messages`, `Authorization: Bearer <TOKEN_IMPORTER_I>`<br>4. `POST /chat/dialogs/direct/<ID_BUYER_B>`, `Authorization: Bearer <TOKEN_BUYER_B>`<br>5. `POST /chat/dialogs/direct/<RANDOM_UUID>`, `Authorization: Bearer <TOKEN_BUYER_B>` |
...
### TC-CHAT-009 — Живой канал принимает access-токен в query и отдаёт кадр `message`; без токена закрывается кодом 4403
...
| Шаги | 1. Подключиться к `ws://localhost:8000/api/v1/chat/ws` без `token`<br>2. Подключиться с `?token=<REFRESH_TOKEN_SELLER_A>` (refresh, не access)<br>3. Подключиться с `?token=<TOKEN_SELLER_A>`; кадры от клиента не нужны<br>4. Не закрывая соединение: `POST /chat/dialogs/<DIALOG_ID>/messages`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело `{"text": "Через канал"}` |
