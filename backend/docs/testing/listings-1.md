...
### TC-LST-001 — Создание пустого черновика
...
### TC-LST-002 — Черновик под привоз только поставщику

| Поле | Значение |
|---|---|
| ID | TC-LST-002 |
| Название | Обычный продавец не может создать объявление вида `import` |
| История | 16 — Профиль поставщика / объявления под заказ |
...
### TC-LST-003 — Создание без токена и лимит черновиков
...
### TC-LST-004 — Заполнение черновика правкой
...
### TC-LST-005 — Пустая правка и неизвестное поле
...
### TC-LST-006 — Чужой и непубличный объявления не видны
...
### TC-LST-007 — Номер телефона в карточке
...
| Тестовые данные | — |
| Шаги | 1. `GET /sale_car/<sale_car_id>` без `Authorization`.<br>2. То же с `Authorization: Bearer <TOKEN_BUYER_B>`.<br>3. То же с `<TOKEN_SELLER_A>`.<br>4. То же с `<TOKEN_MODERATOR_M>`. |
...
### TC-LST-008 — Свои объявления по статусу
...
### TC-LST-009 — Удаление объявления
...
| Шаги | 1. `DELETE /sale_car/<sale_car_id>`, `Authorization: Bearer <TOKEN_BUYER_B>`.<br>2. `DELETE /sale_car/<новый UUID>`, `<TOKEN_SELLER_A>`.<br>3. `DELETE /sale_car/<sale_car_id>`, `<TOKEN_SELLER_A>`.<br>4. `GET /sale_car/<sale_car_id>`, `<TOKEN_SELLER_A>`.<br>5. `GET /sale_car/user`, `<TOKEN_SELLER_A>`. |
