...
### TC-ROLE-001 — Заявка создаётся со статусом pending и видна автору
...
### TC-ROLE-002 — Вторая живая заявка на ту же роль — 409 DUPLICATE_ROLE_REQUEST
...
### TC-ROLE-003 — Гостю — 403 GUEST_FORBIDDEN, без токена — 401

| Поле | Значение |
|---|---|
| ID | TC-ROLE-003 |
| Название | Гостю — 403 GUEST_FORBIDDEN, без токена — 401 |
| История | 13 — Роли и заявки: manager, importer |
...
### TC-ROLE-004 — Одобрение переводит заявку в approved и меняет роль заявителя
...
### TC-ROLE-005 — Отклонение без review_comment — 422 REJECTION_WITHOUT_REASON
...
### TC-ROLE-006 — Повторное решение — 409 ROLE_REQUEST_DECIDED
...
### TC-ROLE-007 — Одобрение заявки на admin модератором — 403 ROLE_ABOVE_REVIEWER
...
### TC-ROLE-008 — Очередь, решение, статистика и role-info закрыты для роли user
...
### TC-ROLE-009 — PUT /role/users/{id}/role: admin — 200, manager — 403, неизвестный id — 404

| Поле | Значение |
|---|---|
| ID | TC-ROLE-009 |
| Название | PUT /role/users/{id}/role: admin — 200, manager — 403, неизвестный id — 404 |
| История | 13 — Роли и заявки: manager, importer; 23 — Админка: люди, карточка, журнал, блокировка |
...
### TC-ROLE-010 — GET /role/stats отдаёт итог и разбивку по ролям
...
| Тестовые данные | `<TOKEN_MODERATOR_M>` — токен именованной записи (README, §2) |
