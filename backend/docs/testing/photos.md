...
### TC-PHO-001 — Первое загруженное фото появляется в галерее и становится обложкой

| Поле | Значение |
|---|---|
| ID | TC-PHO-001 |
| Название | Первое загруженное фото появляется в галерее и становится обложкой |
| История | 05 — Фото объявления: загрузка, порядок, обложка |
...
### TC-PHO-002 — `PUT .../photos/order` задаёт порядок галереи, обложкой становится первое фото
...
### TC-PHO-003 — Удалённое фото исчезает, остальные сохраняют относительный порядок
...
### TC-PHO-004 — Загрузка сверх 15 фото отклоняется целиком с `409 GALLERY_LIMIT_REACHED`
...
| Шаги | 1. `POST /sale_car/<ID>/photos`, `Authorization: Bearer <TOKEN_SELLER_A>`, multipart: `files=@photo15.jpg`, `files=@photo16.jpg`, `files=@photo17.jpg`<br>2. `POST /sale_car/<ID>/photos`, multipart: `files=@photo15.jpg`<br>3. `POST /sale_car/<ID>/photos`, multipart: `files=@photo15.jpg` |
...
### TC-PHO-005 — Файл тяжелее 10485760 байт отклоняется с `413 PHOTO_TOO_LARGE`
...
### TC-PHO-006 — Файл, не являющийся изображением, даёт `422 NOT_AN_IMAGE`, и ни один файл запроса не сохраняется
...
### TC-PHO-007 — В статусе `moderation` добавление и удаление фото дают `409 LISTING_FROZEN`
...
### TC-PHO-008 — Неполный, чужой или повторяющийся список `photo_ids` даёт `422 ORDER_MISMATCH`
...
### TC-PHO-009 — Запрос без файлов даёт `422`, удаление неизвестного `photo_id` — `404 PHOTO_NOT_FOUND`
...
### TC-PHO-010 — Без токена — `401`; не владелец и несуществующий `sale_car_id` — одинаковый `404 LISTING_NOT_FOUND`
