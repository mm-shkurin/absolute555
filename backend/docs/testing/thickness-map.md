...
### TC-THK-001 — Замер капота сохраняется, статус и сводка считаются сервером
...
| Ожидаемый результат | `200`. `sale_car_id` = `<LISTING_ID>`, `measured_panels` = 1, `total_panels` = 13, `is_complete` = false.<br>`measurements[0]`: `panel` = `hood`, `value_um` = 120, `status` = `factory`, `source` = `seller` (если число не совпало с прочитанным со снимка), `photo_url` присутствует (непустая строка), есть `ocr_value_um`, `updated_at`. |
| Статус | Not run |
| Фактический результат | |

### TC-THK-002 — Второй `PUT` на ту же панель не заводит второй замер
...
### TC-THK-003 — Покупатель получает карту опубликованного объявления, сводка есть в карточке

| Поле | Значение |
|---|---|
| ID | TC-THK-003 |
| Название | Покупатель получает карту опубликованного объявления, сводка есть в карточке |
| История | 14 — Карта толщиномера: панели, замеры, фото |
...
| Тестовые данные | `<LISTING_ID>` — `sale_car_id` объявления, опубликованного как в TC-LST-013 |
...
### TC-THK-004 — Чужой пользователь получает 404 на карту и на запись в чужой черновик
...
### TC-THK-005 — Без авторизации — 401, неизвестный `sale_car_id` — 404
...
### TC-THK-006 — Невалидный замер отклоняется 422 и не сохраняется
...
### TC-THK-007 — 13 панелей дают `is_complete` = true, удаление панели снимает флаг
...
| Тестовые данные | `<LISTING_ID>` — черновик, на котором выполнен TC-THK-001 |
| Шаги | 1. `GET /sale_car/<LISTING_ID>/thickness`, заголовок `Authorization: Bearer <TOKEN_SELLER_A>`<br>2. `DELETE /sale_car/<LISTING_ID>/thickness/hood`, тот же заголовок<br>3. `DELETE /sale_car/<LISTING_ID>/thickness/hood`, заголовок `Authorization: Bearer <TOKEN_SELLER_A>` |
...
### TC-THK-008 — `thickness/read` возвращает число, карта не меняется
...
### TC-THK-009 — Без `value_um` или с подтверждённым числом — `ocr`; нечитаемый кадр без числа — 422
...
### TC-THK-010 — Опубликованное объявление принимает исправление замера
