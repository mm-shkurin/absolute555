...
### TC-SUP-001 — Импортёр получает пустой черновик профиля
...
### TC-SUP-002 — Заполненный профиль уходит на модерацию
...
### TC-SUP-003 — Неполный профиль не принимается в очередь
...
### TC-SUP-004 — Профиль на проверке заморожен
...
### TC-SUP-005 — Одобренная витрина видна всем, включая гостя
...
### TC-SUP-006 — Отказ модератора требует причину
...
### TC-SUP-007 — Доступ без роли и без токена
...
### TC-SUP-008 — Отсутствующая витрина и границы страницы
...
### TC-SUP-010 — Правка опубликованной витрины ждёт одобрения
...
### TC-SUP-011 — Объявление под заказ создаёт только импортёр

| Поле | Значение |
|---|---|
| ID | TC-SUP-011 |
| Название | `listing_kind = "import"`: импортёр — 201, продавец — 403 NOT_AN_IMPORTER |
| История | 17 — Объявления под привоз |
...
### TC-SUP-012 — Объявление под заказ: обязательные поля и лента
