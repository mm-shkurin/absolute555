...
### TC-ADM-001 — Список людей отдаётся страницей

| Поле | Значение |
|---|---|
| ID | TC-ADM-001 |
| Название | GET /role/users возвращает страницу и фильтруется |
| История | 23 — Консоль администратора |
...
### TC-ADM-003 — Карточка учётной записи
...
### TC-ADM-004 — Блокировка закрывает доступ по уже выданному токену
...
### TC-ADM-005 — Разблокировка возвращает доступ
...
| Шаги | 1. `POST /role/users/<SELLER_A_ID>/unblock`, `Authorization: Bearer <TOKEN_ADMIN_X>`, тело из данных<br>2. `GET /user/profile`, `Authorization: Bearer <TOKEN_SELLER_A>`<br>3. Повторить шаг 1 |
| Ожидаемый результат | 1. `200`; `is_blocked`=`false`, `blocked_reason`=`null`, `blocked_at`=`null`<br>2. `200`<br>3. `409`; `code` = `ACCESS_UNCHANGED` |
| Статус | Not run |
| Фактический результат | |

### TC-ADM-006 — Отказы блокировки: пустая причина, повтор, сам себя
...
| Шаги | 1. `POST /role/users/<BUYER_B_ID>/block`, `Authorization: Bearer <TOKEN_ADMIN_X>`, тело `{"reason": ""}`<br>2. `POST /role/users/<BUYER_B_ID>/block`, тот же заголовок, тело `{"reason": "спам"}`<br>3. Повторить шаг 2<br>4. `POST /role/users/<ADMIN_X_ID>/block`, тот же заголовок, тело `{"reason": "спам"}` |
...
### TC-ADM-007 — Модератор не блокирует равного и старшего
...
### TC-ADM-008 — Журнал только для admin, консоль закрыта для user
...
### TC-ADM-009 — Заблокированный администратор теряет консоль
...
### TC-ADM-010 — Ушедшая учётная запись отмечена и не блокируется

| Поле | Значение |
|---|---|
| ID | TC-ADM-010 |
| Название | deleted_at в карточке, фильтр deleted, block ушедшего — 409 |
| История | 24 — Консоль: ушедшие учётные записи |
