...
### TC-REC-001 — `POST /sale_car/{id}/sts` принимает скан (202) и ставит чтение в очередь
...
| Шаги | 1. `GET /sale_car/<sale_car_id>`, `Authorization: Bearer <TOKEN_SELLER_A>`.<br>2. `POST /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`, `multipart/form-data`, поле `file` = `sts.jpg`.<br>3. `GET /sale_car/<sale_car_id>`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
| Ожидаемый результат | 1. `200`; `autofill.state: "none"`.<br>2. `202`; `{"sale_car_id": "<sale_car_id>", "autofill": {"state": "pending", ...}}`.<br>3. `200`; `task_id` присутствует (непустой); `autofill.state: "pending"` (или итоговое, если воркер уже закончил), `brand_source` и `model_source` — `null`, пока чтение не завершено. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-002 — Владелец получает истекающую ссылку; новый скан заменяет прежний
...
| Шаги | 1. `GET /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`.<br>2. `POST /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`, multipart, поле `file` = `sts2.png`.<br>3. `GET /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
...
### TC-REC-003 — Чужому — 404, без токена — 401, модератору на модерации — 200
...
| Тестовые данные | `<sale_car_id>` — черновик из TC-REC-001 после FILL, 3 фото и `submit` |
| Шаги | 1. `GET /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_BUYER_B>`.<br>2. `GET /sale_car/<sale_car_id>/sts` без `Authorization`.<br>3. `GET /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_MODERATOR_M>`. |
| Ожидаемый результат | 1. `404`; `code: "LISTING_NOT_FOUND"`.<br>2. `401`; `code: "UNAUTHENTICATED"`.<br>3. `200`; `url` присутствует (непустая строка), `expires_at` — дата в будущем. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-004 — После одобрения ссылка владельцу — 404 `LISTING_NOT_FOUND`
...
| Тестовые данные | `<sale_car_id>` — объявление из TC-REC-003 |
| Шаги | 1. `POST /sale_car/<sale_car_id>/approve`, `Authorization: Bearer <TOKEN_MODERATOR_M>`.<br>2. `GET /sale_car/<sale_car_id>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
| Ожидаемый результат | 1. `200`; `status: "published"`.<br>2. `404`; `code: "LISTING_NOT_FOUND"`. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-005 — Загрузка скана: чужому — 404, без токена — 401, на модерации — 409 `LISTING_FROZEN`
...
| Шаги | 1. `POST /sale_car/<DRAFT_ID>/sts`, `Authorization: Bearer <TOKEN_BUYER_B>`, multipart `file`.<br>2. `POST /sale_car/<DRAFT_ID>/sts` без `Authorization`, multipart `file`.<br>3. `POST /sale_car/<MOD_ID>/sts`, `Authorization: Bearer <TOKEN_SELLER_A>`, multipart `file`. |
| Ожидаемый результат | 1. `404`; `code: "LISTING_NOT_FOUND"`.<br>2. `401`; `code: "UNAUTHENTICATED"`.<br>3. `409`; `code: "LISTING_FROZEN"`. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-006 — `decode-vin` принимает VIN (202), нормализует его и ставит чтение в очередь

| Поле | Значение |
|---|---|
| ID | TC-REC-006 |
| Название | `decode-vin` принимает VIN (202), нормализует его и ставит чтение в очередь |
| История | 20 — Фронт: шаг 2 мастера — распознанное, правка, три состояния |
...
| Шаги | 1. `POST /sale_car/<sale_car_id>/decode-vin`, `Authorization: Bearer <TOKEN_SELLER_A>`, `Content-Type: application/json`, тело из данных.<br>2. `GET /sale_car/<sale_car_id>`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
| Ожидаемый результат | 1. `202`; `sale_car_id` совпадает, `autofill.state: "pending"`.<br>2. `200`; `vin: "XTA21074051234567"`, `task_id` непустой. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-007 — Не-VIN — 422 `VIN_MALFORMED`, задача не ставится
...
| Шаги | 1. Для каждого значения: `POST /sale_car/<sale_car_id>/decode-vin`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело `{"vin": "<значение>"}`.<br>2. `GET /sale_car/<sale_car_id>`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
| Ожидаемый результат | 1. Каждый — `422`; `code: "VIN_MALFORMED"`, `details.vin` — отправленная строка.<br>2. `200`; `autofill.state: "none"`. |
| Статус | Not run |
| Фактический результат | |

### TC-REC-008 — `decode-vin`: чужому — 404, без токена — 401, на модерации — 409 `LISTING_FROZEN`
...
| Шаги | 1. `POST /sale_car/<DRAFT_ID>/decode-vin`, `Authorization: Bearer <TOKEN_BUYER_B>`, тело из данных.<br>2. То же без `Authorization`.<br>3. `POST /sale_car/<MOD_ID>/decode-vin`, `Authorization: Bearer <TOKEN_SELLER_A>`, тело из данных.<br>4. `GET /sale_car/<DRAFT_ID>`, `Authorization: Bearer <TOKEN_SELLER_A>`. |
...
### TC-REC-009 — SSE открыт только владельцу; первое событие — `initial`
...
| Шаги | 1. `GET /task/sse/not-a-uuid` без `Authorization`.<br>2. `GET /task/sse/not-a-uuid`, `Authorization: Bearer <TOKEN_SELLER_A>`.<br>3. `GET /task/sse/<sale_car_id>`, `Authorization: Bearer <TOKEN_BUYER_B>`.<br>4. `GET /task/sse/<sale_car_id>`, `Authorization: Bearer <TOKEN_SELLER_A>`, `Accept: text/event-stream`; держать соединение 35 с. |
