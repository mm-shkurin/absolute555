...
### 4. Страница мастера продажи
Добавлен циклом покрытия (итерация 6). Исполняемая копия:
`frontend/src/features/selling/__tests__/SellingWizardPage.test.tsx`.
```gherkin
Scenario: Вернулся к черновику без цены — мастер открывает шаг цены
...
Scenario: Цена с пробелами сохраняется числом при переходе дальше
Scenario: Продавец добавляет фото и делает второе обложкой
Scenario: Удалённое фото пропадает из шага
Scenario: Без телефона и фото сводка не пускает на модерацию и называет, чего нет
Scenario: Заполненный черновик уходит на модерацию
Scenario: СТС нет под рукой — продавец заполняет характеристики сам
```
